...
public class TransactionsRepository
{
    ...
    public async Task<int> CreateDomestic(TransactionsModel m)
    {
        using var conn = _db.CreateConnection();
        using var cmd = new SqlCommand("sp_CreateDomesticTransaction", conn);
        ...
        cmd.Parameters.Add(
            "@Justify_DateError",
            SqlDbType.DateTime2
        ).Value = m.Justify_DateError ?? (object)DBNull.Value;
    ...
    public async Task<TransactionsModel> GetById(int id)
    {
        ...
        if (await rd.ReadAsync())
        {
            return new TransactionsModel
            {
                ...
                FixedCommission = rd["FixedCommission"] != DBNull.Value ? Convert.ToDecimal(rd["FixedCommission"]) : 0,
                CalculationModeName = rd["CalculationMode"]?.ToString(),
